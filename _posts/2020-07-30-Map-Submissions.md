...
Our friends over at Stratus have kindly offered their map building server, Anax, to be used. For all their info on how to use the server and where to login, check out their [Anax Guide](https://stratus.network/anax). This is probably the easiest place to build a map, they have all the tools you could need ready with just a couple of clicks.

Alternatively, if you're tech savvy, you could set up your own map building localhost server. mitchiii_ wrote up a wonderful guide on how exactly to do this, which he has generously shared with us. (link guide here, just need to make sure it's all up to date with info on 1.8)
